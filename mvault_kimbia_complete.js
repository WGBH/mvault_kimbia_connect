...
var MSB = MSB || (function() {
  ...
  this.stationId = '';
  return {
    init : function(stationData, styleConfig) {
      if (typeof(stationData) === 'string') {
        this.stationId = stationData;
        this.mVaultUrl = stations[stationData].salesForceUrl;
        this.phoneSupport = supportMessaging + stations[stationData].phoneSupport;
      } else if (typeof(stationData) === 'object') {
        this.stationId = stationData.stationId;
        this.mVaultUrl = stationData.mVaultUrl;
        this.phoneSupport = supportMessaging + stationData.phoneSupport;
      } else {
        console.log('Configuration error: please contact an administrator.');
      }
...
function sendObj(obj) {
  ...
  jQuery.ajax({
    ...
    if (retObj['provisioning'] === 'undefined') {
      ...
      showFinalModal("Communications problem with server", MSB.phoneSupport, "Online Support", MSB.helpUrl);
      return false;
    } else {
      ...
      showFinalModal("No response from  server", MSB.phoneSupport, "Online Support", MSB.helpUrl);
  });
}
